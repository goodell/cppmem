...
let is_x_www_form_urlencoded mime_type =
  let x = "application/x-www-form-urlencoded" in
  let n = String.length x in
  if String.length mime_type < n then false
  else (String.sub mime_type 0 n = x)
...
let parse_args () = 
  ...
  let s = 
    if req_method = "GET" || req_method = "HEAD" then
      ...
      if req_method = "POST"
         && is_x_www_form_urlencoded mime_type then begin
